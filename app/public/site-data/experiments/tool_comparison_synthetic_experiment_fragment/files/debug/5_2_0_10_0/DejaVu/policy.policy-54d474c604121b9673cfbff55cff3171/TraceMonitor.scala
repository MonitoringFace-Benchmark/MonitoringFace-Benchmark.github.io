...
/*
  prop fma : !(@ Exists y1 . _P0(y1) & !(_P2 & _P2)) 
  where
    _P0(y1) := e & @ (!e S P0(y1))
    _P2() := e & @ (!e S P2)
...
class Formula_fma(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(10) = build("e")()
      now(14) = build("e")()
      now(15) = build("P0")(V("y1"))
      now(17) = build("e")()
      now(21) = build("e")()
      now(22) = build("P2")()
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(3) = now(4).exist(var_y1.quantvar)
      now(2) = pre(3)
      now(6) = now(7).and(now(8))
      now(5) = now(6).not()
      now(1) = now(2).and(now(5))
      now(0) = now(1).not()
  ...
  val var_y1 :: Nil = declareVariables(("y1",false))(0)

  varsInRelations = Set()
  val indices: List[Int] = List(18,19,11,12,2)

  pre = Array.fill(23)(bddGenerator.False)
  now = Array.fill(23)(bddGenerator.False)

  txt = Array(
    "!(@ Exists y1 . _P0(y1) & !(_P2 & _P2))",
      "@ Exists y1 . _P0(y1) & !(_P2 & _P2)",
      "@ Exists y1 . _P0(y1)",
      "Exists y1 . _P0(y1)",
      "_P0(y1)",
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("e","P2","P0")

  formulae ++= List(new Formula_fma(this))
}
